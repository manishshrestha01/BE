...
export function buildSubjectIntro(semester, subject) {
  ...
  return [
    `In Pokhara University BE Computer Engineering semester ${semesterNumber}, ${subject.name} is not just another syllabus item; it is one of the core knowledge blocks that influences how students perform in later technical courses. A common mistake is to prepare this subject only by reading theory line by line before exams. A stronger method is to combine conceptual reading, short written summaries, and repeated problem-oriented practice from the first month of the term. That approach helps students understand why each unit exists, where questions usually come from, and how to convert chapter-level understanding into scoring answers. The goal of this syllabus is to help you study ${subject.name} in a way that is practical, academic, and aligned with university evaluation style.`,
...
export function buildInternalLinks(semester, subject) {
  ...
  for (const item of related) {
    links.push({ label: `${item.name} syllabus`, href: item.urlPath });
  }
